...
import { listAllNotifications, listAllFollowers, listAllFollowees, respondToRequest, sendFollowRequest } from '../controllers/userController.js';

router.post("/user/follow/:id", auth, sendFollowRequest);
  
  router.get("/user/list-all-notifications", auth, listAllNotifications);
  
  router.patch("/user/respond-to-request/:id/:action", auth, respondToRequest);
